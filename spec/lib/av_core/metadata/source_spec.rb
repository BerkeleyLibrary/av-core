...
module AVCore
  module Metadata
    describe Source do
      ...
      before(:each) do
        @logger_orig = AVCore.instance_variable_get(:@logger)
        AVCore.instance_variable_set(:@logger, Logger.new(File::NULL))
      end

      after(:each) do
        AVCore.instance_variable_set(:@logger, logger_orig)
      end
      ...
      describe Source::MILLENNIUM do
        ...
        describe :record_for do
          ...
          it "raises #{RecordNotFound} if the record cannot be found" do
            stub_request(:get, search_url).to_return(status: 404, body: '')

            expect { Source::MILLENNIUM.record_for('b22139658') }.to raise_error(RecordNotFound)
          end

          it "raises #{RecordNotFound} if the record cannot be parsed" do
            stub_request(:get, search_url).to_return(status: 200, body: 'Something that is not a Millennium MARC HTML page')

            expect { Source::MILLENNIUM.record_for('b22139658') }.to raise_error(RecordNotFound)
          end

          it "raises #{RecordNotFound} if Millennium returns a weird HTTP status" do
            marc_html = File.read('spec/data/b22139658.html')
      ...
      describe Source::TIND do
        ...
        describe :record_for do
          ...
          it "raises #{RecordNotFound} if the record cannot be found" do
            ...
            expect { Source::TIND.record_for('19816') }.to raise_error(RecordNotFound)
          end
          ...
          it "raises #{RecordNotFound} if no records are returned" do
          ...
          it "raises #{RecordNotFound} on a redirect to login" do
          ...
          it "raises #{RecordNotFound} if TIND returns a weird HTTP status" do
            marc_xml = File.read('spec/data/record-19816.xml')
