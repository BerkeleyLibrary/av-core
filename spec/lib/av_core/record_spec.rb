...
module AVCore
  describe Record do
    ...
    describe :from_metadata do
      before(:each) do
        Metadata::Config.tind_base_uri = 'https://digicoll.lib.berkeley.edu'
      end
      ...
      it 'loads the metadata' do
        ...
        record = Record.from_metadata(record_id: '21178', metadata_source: Metadata::Source::TIND)
        expect(record.bib_number).to eq('b23305522')
        ...
        expect(track.duration).to eq(Util::Duration.from_string('00:54:03'))

        fields = record.metadata_fields
        expected = [
